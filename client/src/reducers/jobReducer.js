...
export const getFavoriteJobsReducer = (state = {favoriteJobs: []}, action) => {
  switch (action.type) {
...
export const deleteFavoriteJobReducer = (state = {favoriteJobs: []}, action) => {
  switch (action.type) {
    case DELETE_FAVORITE_JOB_REQUEST:
      return {loadingFavoriteJobs: true, favoriteJobs: action.payload}
    case DELETE_FAVORITE_JOB_SUCCESS:
      return {loadingFavoriteJobs: false, favoriteJobs: action.payload}
    case DELETE_FAVORITE_JOB_FAIL:
      return {loadingFavoriteJobs: false, errorFavoriteJobs: action.payload}
    default:
      return state
  }
...
// get all applicants for selected job reducer
export const getApplicantsReducer = (state = {applicants: []}, action) => {
  switch (action.type) {
    case GET_APPLICANTS_REQUEST:
      return {loadingApplicants: true, state}
    case GET_APPLICANTS_SUCCESS:
      return {loadingApplicants: false, applicants: action.payload}
    case GET_APPLICANTS_FAIL:
      return {loadingApplicants: false, errorApplicants: action.payload}
    default:
      return state
  }
}